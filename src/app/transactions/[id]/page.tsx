...
function SummaryCard({ label, value }: { label: string; value: string }) {
  return (
    <article className="flex min-h-[94px] flex-col justify-center rounded-[20px] border border-light-gray/10 bg-[#090614] p-[15px] font-sf-pro">
...
function DetailRow({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <dd className={`font-bold ${accent ?? "text-[#DADADA]"}`}>{value}</dd>
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-full flex-col gap-[15px] px-[30px] pb-[30px] pt-[46px]">
      ...
      <section className="grid grid-cols-[1fr_282px] gap-[15px] rounded-[12px] border border-light-gray/10 bg-app-black p-[15px]">
        <div>
          <div className="flex min-h-[84px] items-center justify-between rounded-[12px] border border-light-gray/10 bg-[#090614] px-[15px]">
            <div>
              ...
              <p className="mt-1 font-sf-pro text-sm text-[#8E8E93]">Detailed ledger information and system trace for internal audit</p>
            </div>
            <div className="flex gap-2.5">
              <button className="h-11 rounded-[12px] bg-[#2999dc] px-6 font-sf-pro text-base font-bold">Print Ledge</button>
              <button disabled className="flex h-11 items-center gap-3 rounded-[12px] bg-[#2D2D3A] px-[15px] font-sf-pro text-base font-bold text-[#8E8E93]">Download Receipt <Download className="size-5" /></button>
            </div>
          </div>
        ...
        <aside className="rounded-[20px] bg-[#050506] p-[15px]">
          ...
          <div className="space-y-2.5">
            {timeline.map(([title, date], index) => (
              <div key={title} className="flex min-h-[50px] items-center gap-3 rounded-[12px] border border-app-green/20 bg-[#1C1C1E] px-3">
                <span className={`flex size-5 shrink-0 items-center justify-center rounded-full ${index === 3 ? "bg-[#2999dc]" : "bg-[#3BD3A3]"}`}>
                  {index === 3 ? <span className="size-2 rounded-full bg-white" /> : <Check className="size-3 text-[#09281F]" strokeWidth={3} />}
                </span>
                <div className="font-sf-pro"><p className="text-sm font-bold text-[#BEC2DA]">{title}</p><p className="text-xs text-[#8E8E93]">{date}</p></div>
              </div>
            ))}
          </div>
        </aside>
      ...
      <section className="grid grid-cols-[1fr_280px] gap-[15px] rounded-[12px] border border-light-gray/10 bg-app-black p-[15px]">
        ...
        <aside className="rounded-[16px] bg-[#050506] p-[15px]">
          ...
          <div className="space-y-2.5">
            ...
            <button className="flex h-11 w-full items-center justify-center gap-3 rounded-[12px] bg-[#EE171B] font-sf-pro font-bold text-white">Reverse Transaction <RotateCcw className="size-5" /></button>
          </div>
        </aside>
      </section>
      ...
      <section className="grid grid-cols-[0.85fr_0.85fr_1.25fr] gap-[15px]">
        ...
        <article className="rounded-[12px] bg-app-black p-[15px]">
          ...
          <div className="mt-4 flex justify-between px-3 font-sf-pro text-sm font-bold text-[#8E8E93]"><span>Description</span><span>Amount</span></div>
          <div className="mt-5 space-y-2.5 font-sf-pro text-sm font-bold">
            <div className="flex h-[45px] items-center justify-between rounded-[12px] bg-[#252331] px-[15px] text-[#8E8E93]"><span>Amount Paid</span><span className="text-[#DADADA]">₦12,500</span></div>
            <div className="flex h-[45px] items-center justify-between rounded-[12px] bg-[#252331] px-[15px] text-[#8E8E93]"><span>Platform Fee</span><span className="text-[#DADADA]">₦125</span></div>
            <div className="flex h-[45px] items-center justify-between rounded-[12px] bg-[#086BF2] px-[15px] text-white"><span>Net Business Receives</span><span>₦12,625</span></div>
          </div>
        </article>
      </section>
    </div>
