  ...
  return (
    <div className="flex min-h-full flex-col gap-[15px] px-[30px] pb-[30px] pt-[46px]">
      ...
      <section className="grid min-h-[330px] grid-cols-[1fr_282px] gap-[15px] rounded-[12px] border border-light-gray/10 bg-app-black p-[15px]">
        <div className="flex flex-col gap-[18px]">
          <div className="flex min-h-[84px] items-center justify-between rounded-[12px] border border-light-gray/10 bg-[#090614] px-[15px]">
            <div>
              ...
              <p className="mt-1 font-sf-pro text-sm text-[#8E8E93]">
                {order.businessId}
                <span className="ml-2">
                  Business: <b className="text-base text-[#BEC2DA]">{order.business}</b>
                </span>
              </p>
            </div>
            <div className="flex gap-2.5">
              <button className="h-11 rounded-[12px] bg-[#2999dc] px-6 font-sf-pro text-base font-bold text-white">
                Accept Order
              </button>
          ...
          <div className="grid flex-1 grid-cols-2 gap-[15px]">
            <article className="rounded-[20px] border border-light-gray/10 bg-[#090614] p-[15px]">
              ...
              <dl className="space-y-2 font-sf-pro text-sm">
                <div className="flex justify-between">
                  <dt className="text-[#8E8E93]">Model</dt>
                  <dd className="text-base font-bold text-[#BEC2DA]">{order.model}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-[#8E8E93]">Quantity</dt>
                  <dd className="text-base font-bold text-[#BEC2DA]">{order.quantity}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-[#8E8E93]">Unit Price</dt>
                  <dd className="text-base font-bold text-[#BEC2DA]">{order.unitPrice}</dd>
                </div>
              </dl>
              <div className="my-[14px] h-px bg-light-gray/10" />
              <div className="flex justify-between font-sf-pro text-sm">
                <span className="text-[#8E8E93]">Total Price</span>
            ...
            <article className="rounded-[20px] border border-light-gray/10 bg-[#090614] p-[15px]">
              ...
              <dl className="space-y-3 font-sf-pro text-sm">
                <div className="flex justify-between">
                  <dt className="text-[#8E8E93]">Transaction ID</dt>
                  <dd className="text-base font-bold text-[#BEC2DA]">{order.transactionId}</dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt className="text-[#8E8E93]">Payment Status</dt>
                  <dd className="flex items-center gap-2 text-base font-bold text-app-green">
                    <span className="flex size-5 items-center justify-center rounded-full bg-app-green text-[#090614]">
              ...
              <div className="mt-6 flex h-[45px] items-center justify-between rounded-[12px] bg-app-blue/15 px-[15px] font-sf-pro">
                <span className="text-sm text-[#8E8E93]">Total Price</span>
                <span className="text-base font-bold text-app-blue">{order.totalPrice}</span>
              </div>
            </article>
          </div>
        ...
        <aside className="flex flex-col rounded-[20px] bg-[#050506] p-[15px]">
          <h2 className="mb-4 flex items-center gap-2 font-sf-pro text-base font-bold text-white">
          ...
          <div className="space-y-3 font-sf-pro text-sm">
            <div>
              <p className="text-[#8E8E93]">Recipient</p>
              <p className="text-base font-bold text-[#BEC2DA]">{order.recipient}</p>
              <p className="text-[#8E8E93]">{order.phone}</p>
            </div>
            <div>
              <p className="text-[#8E8E93]">Shipping Address</p>
              <p className="text-base font-bold text-[#BEC2DA]">{order.location}</p>
              <p className="text-[#8E8E93]">{order.address}</p>
            </div>
            <div>
              <p className="text-[#8E8E93]">Tracking Number</p>
              <p className="text-base font-bold text-[#BEC2DA]">{order.trackingNumber}</p>
            </div>
          </div>
          <button
            disabled
            className="mt-auto flex h-[40px] items-center justify-center gap-2 rounded-[12px] bg-[#2D2D2E] font-sf-pro text-base font-bold text-[#8E8E93]"
          >
      ...
      <section className="rounded-[12px] border border-light-gray/10 bg-app-black p-2.5">
        <h2 className="mb-3 font-sf-pro text-base font-bold text-[#BEC2DA]">Activity Log</h2>
        <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
          {orderActivity.map((activity, index) => (
            <div key={`${activity}-${index}`} className="flex items-center gap-3 font-sf-pro text-xs">
              <span className="shrink-0 text-[#8E8E93]">13:10/11 Jan 2026</span>
              <span className="flex-1 rounded-[7px] border border-light-gray/10 bg-[#090614] px-2 py-1.5 text-app-yellow">
                {activity}
              </span>
            </div>
          ))}
