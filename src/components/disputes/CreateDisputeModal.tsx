  ...
  return (
    <>
      <div className="flex gap-2.5">
        ...
        <button type="button" onClick={() => setOpen(true)} className="h-11 rounded-[12px] border border-app-blue bg-[#2999dc] px-6 font-sf-pro text-base font-bold text-white">
          Create Dispute
        </button>
      </div>
      ...
      {open && (
        ...
        >
          <section role="dialog" aria-modal="true" aria-labelledby="create-dispute-title" className="w-full max-w-[666px] rounded-[20px] border border-light-gray/20 bg-[#020203] p-[14px] shadow-2xl">
            ...
            <form className="space-y-3.5" onSubmit={(event) => event.preventDefault()}>
              ...
              <label className="block font-sf-pro">
                <span className="mb-2 block text-sm text-[#BEC2DA]">Description/Reason</span>
                <textarea
                  aria-label="Description or reason"
                  placeholder="Provide a detailed explanation for this sovereign audit exception"
                  className="h-[96px] w-full resize-none rounded-[12px] border border-light-gray/15 bg-[#101011] px-3 py-2.5 text-sm text-white outline-none placeholder:text-[#8E8E93] focus:border-app-blue"
                />
              </label>
...
function ModalField({ label, value, accent = false, select = false }: { label: string; value: string; accent?: boolean; select?: boolean }) {
  return (
    <label className="block min-w-0 flex-1 font-sf-pro">
      ...
      <span className={`flex h-[44px] items-center justify-between rounded-[12px] border border-light-gray/15 bg-[#101011] px-3 text-sm ${accent ? "text-app-blue" : "text-[#8E8E93]"}`}>
        {value}
        {select && <ChevronDown className="size-5 text-[#BEC2DA]" />}
      </span>
    </label>
  );
}
